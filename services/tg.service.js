...
module.exports = ({ phone }) => {
    ...
    return {
        ...
        send_message: async function ({ users, text }) {
        ...
        send_file: async function ({ files, users }) {
            const user_list = users.split(",");
            for (const user of user_list) {
                const chat = await client.invoke({
                    _: "searchPublicChat",
                ...
                for (const [i, file] of files.entries()) {
                    await client.invoke({
                        ...
                        input_message_content: {
                            ...
                            caption: {
                                _: 'formattedText',
                                text: i < 1 ? `Привет, отправил файл (${file.caption}) в нем хранятся все данные по таблице` : null,
                            }
                        }
                    });
                }
